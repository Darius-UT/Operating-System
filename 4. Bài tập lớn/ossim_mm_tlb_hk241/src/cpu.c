...
int run(struct pcb_t * proc, int *countTLB,int *countTLBMiss) {
	...
	case READ:
#ifdef CPU_TLB
		stat = tlbread(proc, ins.arg_0, ins.arg_1, ins.arg_2,countTLB,countTLBMiss);
#elif defined(MM_PAGING)
	...
	case WRITE:
#ifdef CPU_TLB
		stat = tlbwrite(proc, ins.arg_0, ins.arg_1, ins.arg_2,countTLB,countTLBMiss);
#elif defined(MM_PAGING)
