  ...
  { /* Page is not online, make it actively living */
    ...
    if (MEMPHY_get_freefp(caller->mram, &vicfpn) < 0) // RAM doesn't have any free frame -> Paging
    {
      ...
      for (index_of_active_mswp = 0; index_of_active_mswp < PAGING_MAX_MMSWP; index_of_active_mswp++) {
        if (caller->mswp[index_of_active_mswp] == caller->active_mswp) break;
      }
      ...
      if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn) < 0) {
        ...
        if (i == PAGING_MAX_MMSWP) return -1; // MEMSWAP doesn't have any free frame
      }
    ...
    __swap_cp_page(caller->mswp[swptyp], swpoff, caller->mram, vicfpn);

    MEMPHY_put_freefp(caller->mswp[swptyp], swpoff);
...
int validate_overlap_vm_area(struct pcb_t *caller, int vmaid, int vmastart, int vmaend)
{
  ...
  return OVERLAP(vma->vm_start, vma->vm_end, vmastart, vmaend)?1:-1;
}
...
{
  //printf("Bi nulll o get_free_vmrg_area\n");
  struct vm_area_struct *cur_vma = get_vma_by_num(caller->mm, vmaid);
