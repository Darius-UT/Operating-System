...
/* EXtract Usr Number */
#define PAGING_USRNUM(x)  GETVAL(x, PAGING_PTE_USRNUM_MASK,PAGING_PTE_USRNUM_LOBIT)
...
//int enlist_pgn_node(struct pgn_t **pgnlist, int pgn); old
...
int tlbwrite(struct pcb_t * proc, BYTE data, uint32_t destination, uint32_t offset,int* countTLB, int* countTLBMiss);
int init_tlbmemphy(struct memphy_struct *mp, int max_size);
void init_tlb_cache(struct tlb_entry *mp, int maxsize);
int TLBMEMPHY_read(struct memphy_struct * mp, int addr, BYTE *value);
int TLBMEMPHY_write(struct memphy_struct * mp, int addr, BYTE data);
int TLBMEMPHY_dump(struct memphy_struct * mp);
int tlb_cache_read(struct memphy_struct * mp, int pid, int pgnum,struct tlb_entry* tlb_cache);
